...
if not setfflag or 
    ...
    not debug.getupvalue or
    not debug.setupvalue or
    not firesignal or
    not checkcaller or 
    not getgc or 
...
local ACHAOTICDATA = {
    ...
    TrainingBalls = Workspace:WaitForChild("TrainingBalls"),
    Parry = {},
    Global = {
        LastInput = nil,
...
local RuntimeState = {
    Unloaded = false,
    OldIndex = nil,
    OriginalPhysicsSenderRate = nil,
    ImmortalityFlagApplied = false,
    ImmortalityRestorePending = false,
    ImmortalityRoot = nil,
    ImmortalityCFrame = nil,
    ImmortalityVelocity = nil,
}
...
local function StopAnimation(animationtrack, FadeTime)
    ...
    local StopFadeTime = FadeTime
    if StopFadeTime == nil then
        StopFadeTime = animationtrack:GetAttribute("StopFadeTime")
    end
    animationtrack:Stop(tonumber(StopFadeTime) or 0)
end

local function PlayGrabAnimation(animationtrack)
    if not animationtrack then return end
    local PlayFadeTime = tonumber(animationtrack:GetAttribute("PlayFadeTime")) or 0
    local PlayWeight = tonumber(animationtrack:GetAttribute("PlayWeight")) or 1
    local PlaySpeed = tonumber(animationtrack:GetAttribute("PlaySpeed")) or 1
    animationtrack:Play(PlayFadeTime, PlayWeight, PlaySpeed)
end

local function IsParryAllowed()
    ...
    local IsInLobbyTraining = LocalPlayer:GetAttribute("LobbyTraining") and Character.Parent == Workspace.Dead
    local IsLobbyContext = ACHAOTICASSETS.DebugFlags.LobbyParry or LocalPlayer:GetAttribute("LobbyParry") or IsInLobbyTraining

    if Character.Parent ~= Workspace.Alive and not IsLobbyContext then
        return false, IsInLobbyTraining
    end

    local QiLevel = math.huge
    local Upgrades = LocalPlayer:FindFirstChild("Upgrades")
    local QiCharge = Upgrades and Upgrades:FindFirstChild("Qi-Charge")
    if QiCharge then
        QiLevel = QiCharge.Value
    end

    if Character:GetAttribute("DoNotParry") then
        return false, IsInLobbyTraining
    end

    if Character:GetAttribute("ChargingAdrenaline") and QiLevel < 2 then
        return false, IsInLobbyTraining
    end
...
local function PlayParry_Animation()
    ...
    local IsAllowed = IsParryAllowed()
    if ACHAOTICDATA.Config.ParrySettings.SafeMode and not IsAllowed then
        return
    end
    ...
    ACHAOTICDATA.Config.Animation.GrabParry = track
    PlayGrabAnimation(track)
end
...
local function GetParry_Data(curveDirection, IsInLobbyTrainning)
    ...
    if IsInLobbyTrainning then
        ...
        if CurrentlySelectedMode == "Hovergoal" or CurrentlySelectedMode == "Soccer" then
            ...
            for _, hovergoalgoal in CollectionService:GetTagged("HovergoalGoal") do
                if hovergoalgoal.Name == ("Goal%s"):format(tostring(TeamNumber)) then
    ...
    elseif curveDirection == 'Straight' then
        ...
        local Character = GetCharacter()
        local CharacterRoot = Character and (Character.PrimaryPart or Character:FindFirstChild("HumanoidRootPart"))
        local TargetPosition
        if closestEntity then
            if closestEntity:IsA("BasePart") then
                TargetPosition = closestEntity.Position
            elseif closestEntity.PrimaryPart then
                TargetPosition = closestEntity.PrimaryPart.Position
            end
        end

        if CharacterRoot and TargetPosition then
            return {CFrame.new(CharacterRoot.Position, TargetPosition), PlayerPositions, Vector2_Mouse_Location}
        end
        return {ACHAOTICASSETS.CurrentCamera.CFrame, PlayerPositions, Vector2_Mouse_Location}
...
local ExecuteRemoteFireServer = function(ParryData)
    local ParryRemote = ParryDATA.ParryRemote
    local ParryFunction = ParryDATA.ParryFunction
    if not ParryRemote or not ParryFunction or not ParryData then
        return false
    end

    local ParryUUID = debug.getupvalue(ParryFunction, 8)
    local HashTable = debug.getupvalue(ParryFunction, 5)
    local ParryHash = type(HashTable) == "table" and HashTable[3] or nil
    if ParryUUID == nil or ParryHash == nil then
        return false
    end
...
local FireParry = function()
    ...
    if ParryMethod == "Blatant" then
        local IsAllowed, IsInLobbyTraining = IsParryAllowed()
        if ACHAOTICDATA.Config.ParrySettings.SafeMode and not IsAllowed then
            return false
        end

        local ParryData = GetParry_Data(ACHAOTICDATA.Config.ParrySettings.ParryCurveDirection, IsInLobbyTraining)
        Fired = ExecuteRemoteFireServer(ParryData)
    elseif ParryMethod == "Legit" then
        local PlayerGui = ACHAOTICDATA.Player.LocalPlayer:FindFirstChildOfClass("PlayerGui")
    ...
    if Fired then
        ACHAOTICDATA.Global.lastParryTime = os.clock()
        if ACHAOTICDATA.Global.Parries <= 7 then
            ACHAOTICDATA.Global.Parries += 1
            task.delay(0.5, function()
                if not RuntimeState.Unloaded and ACHAOTICDATA.Global.Parries > 0 then
                    ACHAOTICDATA.Global.Parries -= 1
                end
            end)
        end
...
local function GetTraining_Balls()
    local BallInstances = {}
    for _, Instance in pairs(ACHAOTICDATA.TrainingBalls:GetChildren()) do
        if Instance:GetAttribute("realBall") then
            table.insert(BallInstances, Instance)
        end
    end
...
local function GetBallState(Ball)
    local State = ACHAOTICDATA.Parry[Ball]
    if State then
        return State
    end
    ...
    State = {
        ...
        AutoParriedUntil = 0,
        LobbyParriedUntil = 0,
        TriggerParriedUntil = 0,
        TargetConn = nil,
    }

    State.TargetConn = Ball:GetAttributeChangedSignal("target"):Connect(function()
        State.AutoParriedUntil = 0
        State.LobbyParriedUntil = 0
        State.TriggerParriedUntil = 0
        State.lastVelUnit = nil
        ACHAOTICDATA.Global.AutoParryParried = false
        TriggerBotParried = false
    end)

    ACHAOTICDATA.Parry[Ball] = State
    return State
end

local function IsBall_Curved(Ball, Ping, Character)
    ...
    if not Root or not Zoomies then
        return false
    end
    ...
    local TowardDot = ToPlayer.Unit:Dot(BallDirection)
    local PreviousDirection = State.lastVelUnit or BallDirection
    local SampleDelta = math.max(Now - State.lastVelocitySample, 1 / 240)
    local DirectionDot = math.clamp(PreviousDirection:Dot(BallDirection), -1, 1)
    local DirectionAngle = math.acos(DirectionDot)
    local TurnRate = DirectionAngle / SampleDelta

    State.lastVelUnit = BallDirection
    State.lastVelocitySample = Now
    State.turnRate = GetLinear_Interpolation(State.turnRate or 0, TurnRate, 0.25)
    State.lerp_radians = GetLinear_Interpolation(State.lerp_radians or 0, math.acos(math.clamp(TowardDot, -1, 1)), 0.2)

    local Latency = math.max(Ping, 0) / 1000
    local ReachTime = Distance / Speed
    local NearDistance = math.max(12, Speed * (0.055 + Latency * 0.35))
    if Distance <= NearDistance then
        return false
    end

    local TowardThreshold = 0.72 - math.clamp(Latency * 0.2, 0, 0.12)
    if TowardDot < TowardThreshold then
        return true
    end

    local TurnThreshold = math.rad(42 + math.clamp(Speed / 35, 0, 28))
    if ReachTime > Latency + 0.06 and State.turnRate > TurnThreshold then
        State.curving = Now
        return true
    end

    if Now - (State.curving or 0) < math.min(0.18, ReachTime * 0.45) then
        return true
    end
...
local function GetClosest_Player()
    ...
    local Root = lpCharacter and (lpCharacter.PrimaryPart or lpCharacter:FindFirstChild("HumanoidRootPart"))
    if not Root then
        return nil
    end

    for _, entity in ipairs(Workspace.Alive:GetChildren()) do
        local EntityRoot = entity.PrimaryPart or entity:FindFirstChild("HumanoidRootPart")
        if entity ~= lpCharacter and EntityRoot then
...
local function GetSpamThreshold(Ping, BallSpeed)
    local PingFactor = math.max(Ping, 0) / 500
    local Divisor, Minimum, Maximum

    if Ping >= 225 then
        Divisor, Minimum, Maximum = 4, 32.5, 80
    elseif Ping >= 160 then
        Divisor, Minimum, Maximum = 4.25, 29.5, 70
    elseif Ping >= 130 then
        Divisor, Minimum, Maximum = 4.65, 27.25, 70
    else
        Divisor, Minimum, Maximum = 5, 26, 70
    end

    return math.clamp(BallSpeed / Divisor + PingFactor, Minimum, Maximum)
end

local function MainConnection()
    if RuntimeState.Unloaded then return end
    ...
    if ACHAOTICDATA.Config.AutoSpamParry.Enabled then
        local ClosestEntity = GetClosest_Player()
        local ClosestRoot = ClosestEntity and (ClosestEntity.PrimaryPart or ClosestEntity:FindFirstChild("HumanoidRootPart"))
        local ShouldSpam = false
        local HighestRange = 0

        if ClosestRoot then
            local DistToTarget = (Root.Position - ClosestRoot.Position).Magnitude
            for _, Ball in ipairs(BallsList) do
                local Zoomies = Ball:FindFirstChild("zoomies")
                if Zoomies then
                    local TargetCheck = GetSpamThreshold(Ping, Zoomies.VectorVelocity.Magnitude)
                    local DistToBall = (Root.Position - Ball.Position).Magnitude
                    local DetectionValid

                    if ACHAOTICDATA.Config.AutoSpamParry.DetectionMode == "Speed" then
                        DetectionValid = ACHAOTICDATA.Global.Parries >= math.max(1, ACHAOTICDATA.Config.AutoSpamParry.Threshold)
                    else
                        DetectionValid = true
                    end

                    HighestRange = math.max(HighestRange, TargetCheck * 1.5)
                    if DetectionValid and DistToBall <= TargetCheck * 1.5 and DistToTarget <= TargetCheck then
                        ShouldSpam = true
                    end
                end
            end
        ...
        ACHAOTICDATA.Config.AutoSpamParry.Spamming = ShouldSpam
        ACHAOTICDATA.Global.AutoSpamParryCurrentAccuracy = HighestRange
    else
        ACHAOTICDATA.Config.AutoSpamParry.Spamming = false
        ACHAOTICDATA.Global.AutoSpamParryCurrentAccuracy = 0
    end
    ...
    if ACHAOTICDATA.Config.AutoParry.Enabled and not IsSpamming and not ACHAOTICDATA.Config.TriggerBot.Enabled then
        local HighestAccuracy = 0
        for _, Ball in ipairs(BallsList) do
            local Zoomies = Ball:FindFirstChild("zoomies")
            if not Zoomies then continue end

            local State = GetBallState(Ball)
            if Now < State.AutoParriedUntil then continue end

            local BallTarget = Ball:GetAttribute("target")
            if BallTarget ~= tostring(ACHAOTICDATA.Player.LocalPlayer) then continue end

            local Velocity = Zoomies.VectorVelocity
            local Distance = (Root.Position - Ball.Position).Magnitude
            local PingThreshold = math.clamp(Ping / 20, 5, 17)
            local Speed = Velocity.Magnitude
            local CappedSpeedDiff = math.min(math.max(Speed - 9.5, 0), 650)
            local AccuracyMultiplier = ACHAOTICDATA.Config.ParrySettings.SpeedDivisorMultiplier
            if ACHAOTICDATA.Config.ParrySettings.AutoParryAutoAccuracy then
                AccuracyMultiplier = math.clamp(1.05 - (Ping / 1000) * 0.18 - (Speed / 1000) * 0.2, 0.72, 1.05)
            end
            local SpeedDivisor = (2.4 + CappedSpeedDiff * 0.002) * AccuracyMultiplier
            local ParryAccuracy = PingThreshold + math.max(Speed / math.max(SpeedDivisor, 0.05), 9.5)
            HighestAccuracy = math.max(HighestAccuracy, ParryAccuracy)
            ...
            if ACHAOTICDATA.Config.AutoParry.AntiCurveEnabled and IsBall_Curved(Ball, Ping, Character) then continue end

            if GlobalEnvironment.CooldownProtection then
                local ok, ParryCD = pcall(function()
                    return ACHAOTICDATA.Player.LocalPlayer.PlayerGui.Hotbar.Block.UIGradient
                end)
            ...
            if Distance <= ParryAccuracy then
                if Now - State.LastParry > 0.25 and ACHAOTICDATA.Config.AutoParry.AnimationFix and ACHAOTICDATA.Config.ParrySettings.ParryMethod ~= "Legit" then
                    PlayParry_Animation()
                end

                if FireParry() then
                    State.LastParry = Now
                    State.AutoParriedUntil = Now + 0.85
                    ACHAOTICDATA.Global.AutoParryParried = true
                end
            end
        end
        ACHAOTICDATA.Global.AutoParryCurrentAccuracy = HighestAccuracy
    else
        ACHAOTICDATA.Global.AutoParryCurrentAccuracy = 0
    end

    if ACHAOTICDATA.Config.LobbyAutoParry.Enabled then
        for _, Ball in ipairs(GetTraining_Balls()) do
            local Zoomies = Ball:FindFirstChild("zoomies")
            if not Zoomies then continue end

            local State = GetBallState(Ball)
            if Now < State.LobbyParriedUntil then continue end
            if Ball:GetAttribute("target") ~= tostring(ACHAOTICDATA.Player.LocalPlayer) then continue end

            local Velocity = Zoomies.VectorVelocity
            local Distance = math.max((Root.Position - Ball.Position).Magnitude - 5, 0)
            local Speed = Velocity.Magnitude * 1.5
            local CappedSpeedDiff = math.min(math.max(Speed - 9.5, 0), 650)
            local SpeedDivisor = (2.4 + CappedSpeedDiff * 0.002) * ACHAOTICDATA.Config.ParrySettings.LobbySpeedDivisorMultiplier
            local ParryAccuracy = math.clamp(Ping / 20, 5, 17) + math.max(Speed / math.max(SpeedDivisor, 0.05), 9.5) + Distance / 75
            local Curved = ACHAOTICDATA.Config.LobbyAutoParry.AntiCurveEnabled and IsBall_Curved(Ball, Ping, Character)

            if Distance <= ParryAccuracy and not Curved then
                if Now - State.LobbyLastParry > 0.25 and ACHAOTICDATA.Config.LobbyAutoParry.AnimationFix and ACHAOTICDATA.Config.ParrySettings.ParryMethod ~= "Legit" then
                    PlayParry_Animation()
                end

                if FireParry() then
                    State.LobbyLastParry = Now
                    State.LobbyParriedUntil = Now + 0.85
                end
            end
        end
    end
    ...
    if ACHAOTICDATA.Config.TriggerBot.Enabled and not IsSpamming then
        for _, Ball in ipairs(BallsList) do
            local State = GetBallState(Ball)
            if Now < State.TriggerParriedUntil then continue end
            if Ball:GetAttribute("target") ~= tostring(ACHAOTICDATA.Player.LocalPlayer) then continue end
            if Root:FindFirstChild("SingularityCape") then continue end
            ...
            if ACHAOTICDATA.Config.TriggerBot.AnimationFix and ACHAOTICDATA.Config.ParrySettings.ParryMethod ~= "Legit" then
                PlayParry_Animation()
            end

            if FireParry() then
                State.TriggerParriedUntil = Now + 0.85
...
NeverZen:Track(ACHAOTICDATA.Balls.ChildRemoved:Connect(function(Child)
    if ACHAOTICDATA.Parry[Child] then
        local TargetConn = ACHAOTICDATA.Parry[Child].TargetConn
        if TargetConn then
...
NeverZen:Track(ACHAOTICDATA.TrainingBalls.ChildRemoved:Connect(function(Child)
    if ACHAOTICDATA.Parry[Child] then
        local TargetConn = ACHAOTICDATA.Parry[Child].TargetConn
        if TargetConn then
            pcall(function() TargetConn:Disconnect() end)
...
local function MakeDraggable(Recv, update, speed)
    ...
    NeverZen:Track(Recv.InputBegan:Connect(function(input)
        if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) then 
            dragToggle = true
            dragStart = input.Position
            startPos = update.Position
            NeverZen:Track(input.Changed:Connect(function()
                if input.UserInputState == Enum.UserInputState.End then
                    dragToggle = false
                end
            end))
...
function Visuals.VisualiserService:SetColor(color)
    for _, Part in pairs(Visuals.VisualParts) do
        if Part and Part.Parent and Part.Color ~= color then
            Part.Color = color
        end
    end
end
...
function Visuals.VisualiserService:Update(hrp, position, size)
    ...
    visualPart.Size = visualPart.Size:Lerp(TargetSize, 0.25)
    visualPart.Transparency = Visuals.VisualisersEnabled and 0 or 1
end
...
do
    ...
    do
        ...
        AutoParrySection:AddSlider({
            ...
            Type = "%",
            Callback = function(value)
                ACHAOTICDATA.Config.ParrySettings.AutoParryAccuracy = value
    ...
    do
        ...
        LobbyAutoParrySection:AddSlider({
            ...
            Type = "%",
            Callback = function(value)
                ACHAOTICDATA.Config.ParrySettings.LobbyAutoParryAccuracy = value
...
do
    ...
    UI.StatsUI.States.CPU.Text = "0.0ms"
...
do
    ...
    if getfflag then
        pcall(function()
            RuntimeState.OriginalPhysicsSenderRate = getfflag("S2PhysicsSenderRate")
        end)
    end
    RuntimeState.OriginalPhysicsSenderRate = RuntimeState.OriginalPhysicsSenderRate or "15"

    local function SetPhysicsSenderRate(enabled)
        if enabled and not RuntimeState.ImmortalityFlagApplied then
            pcall(setfflag, "S2PhysicsSenderRate", "1333335")
            RuntimeState.ImmortalityFlagApplied = true
        elseif not enabled and RuntimeState.ImmortalityFlagApplied then
            pcall(setfflag, "S2PhysicsSenderRate", tostring(RuntimeState.OriginalPhysicsSenderRate))
            RuntimeState.ImmortalityFlagApplied = false
        end
    end

    NeverZen:Track(RunService.PreSimulation:Connect(function()
        if RuntimeState.Unloaded then return end

        local Character = GetCharacter()
        local hrp = Character and Character:FindFirstChild("HumanoidRootPart")
        if not Immortality.Enabled or not hrp then
            SetPhysicsSenderRate(false)
            return
        end

        SetPhysicsSenderRate(Immortality.SpeedBypassEnabled)

        DesyncTypes[1] = hrp.CFrame
        DesyncTypes[2] = hrp.AssemblyLinearVelocity
        RuntimeState.ImmortalityRoot = hrp
        RuntimeState.ImmortalityCFrame = DesyncTypes[1]
        RuntimeState.ImmortalityVelocity = DesyncTypes[2]
        RuntimeState.ImmortalityRestorePending = true
        ...
        hrp.CFrame = DesyncTypes[1] + CalculatedOffset
        hrp.AssemblyLinearVelocity = Vector3.zero
    end))

    NeverZen:Track(RunService.PostSimulation:Connect(function()
        if not RuntimeState.ImmortalityRestorePending then return end

        local hrp = RuntimeState.ImmortalityRoot
        if hrp and hrp.Parent then
            hrp.CFrame = RuntimeState.ImmortalityCFrame
            hrp.AssemblyLinearVelocity = RuntimeState.ImmortalityVelocity
        end
        RuntimeState.ImmortalityRestorePending = false
    end))

    local oldIndex
    oldIndex = hookmetamethod(game, "__index", newcclosure(function(self, key)
        if not RuntimeState.Unloaded and Immortality.Enabled and not checkcaller() and key == "CFrame" then
            local Character = GetCharacter()
            local hrp = Character and Character:FindFirstChild("HumanoidRootPart")
            if hrp then
                if self == hrp then
                ...
                if self == Head and DesyncTypes[1] then
                    return DesyncTypes[1] + Vector3.new(0, hrp.Size.Y / 2 + 0.5, 0)
                end
            end
        end
        return oldIndex(self, key)
    end))
    RuntimeState.OldIndex = oldIndex

    table.insert(NeverZen.Cleanups, function()
        RuntimeState.Unloaded = true
        SetPhysicsSenderRate(false)

        if RuntimeState.ImmortalityRestorePending then
            local hrp = RuntimeState.ImmortalityRoot
            if hrp and hrp.Parent then
                hrp.CFrame = RuntimeState.ImmortalityCFrame
                hrp.AssemblyLinearVelocity = RuntimeState.ImmortalityVelocity
        ...
        if RuntimeState.OldIndex then
            pcall(function()
                hookmetamethod(game, "__index", RuntimeState.OldIndex)
            end)
            RuntimeState.OldIndex = nil
        end
    end)
end
...
do 
    do
        do
            ...
            SkinChanger.System.OriginalSetSword = OriginalSetSword
...
do 
    ...
    NeverZen:Track(ReplicatedStorage.Packages._Index["sleitnick_net@0.1.0"].net["RE/SlashesOfFuryCatch"].OnClientEvent:Connect(function()
        task.spawn(function()
            while not RuntimeState.Unloaded and ACHAOTICDATA.Config.ParrySettings.Detections.SlashesofFury.Flag and ACHAOTICDATA.Config.ParrySettings.Detections.SlashesofFury.Count < ACHAOTICDATA.Config.ParrySettings.SlashesofFuryDetectionMaxParryCount do
...
local function RefreshVM(deltaTime)
    ...
    if ACHAOTICDATA.Config.ClientStats.Enabled then
        ...
        Cpu = math.max(deltaTime, 0) * 1000

        UI.StatsUI.States.FPS.Text = tostring(math.floor(Fps))
        UI.StatsUI.States.PING.Text = string.format("%sms", tostring(math.floor(Ping)))
        UI.StatsUI.States.CPU.Text = string.format("%.1fms", Cpu)
        UI.StatsUI.States.MEMORY.Text = string.format("%smb", tostring(math.floor(Memory)))
    end
end

local RequestSpamParry = function()
    local AutoSpamming = ACHAOTICDATA.Config.AutoSpamParry.Spamming
    local ManualSpamming = ACHAOTICDATA.Config.ManualSpamParry.Spamming
    if RuntimeState.Unloaded or not (AutoSpamming or ManualSpamming) then
        return false
    end
    ...
    local ShouldAnimate = (
        (ACHAOTICDATA.Config.AutoSpamParry.AnimationFix and AutoSpamming) or
        (ACHAOTICDATA.Config.ManualSpamParry.AnimationFix and ManualSpamming)
    ) and ACHAOTICDATA.Config.ParrySettings.ParryMethod ~= "Legit"

    if Fired and ShouldAnimate then
        SpamParry_Animation()
    end
    return Fired
end
...
do
    ...
    NeverZen:Track(RunService.RenderStepped:Connect(function(deltaTime)
        ...
        if not IsSpamming then
            timeAccumulator = 0
            if UI.SpamAccumulatorLabel then
                UI.SpamAccumulatorLabel:SetValue("Accumulator: 0.000")
            end
            return
        end

        local TargetSpeed = math.clamp(ACHAOTICDATA.Config.ParrySettings.SpammingRPS, 1, 240)
        timeAccumulator += math.min(deltaTime, 0.1)
        local packetsToFire = math.min(math.floor(timeAccumulator * TargetSpeed), 8)
...
UnloadACHT = function()
    ...
    local ExtraGuis = {
        UI.StatsUI.ScreenGui,
        UI.BallStats.ScreenGui,
        UI.SpamUI.ScreenGui,
        UI.TriggerBotUI.ScreenGui,
        UI.ImmortalityUI.ScreenGui,
    }

    NeverZen:Unload()

    for _, Gui in ipairs(ExtraGuis) do
