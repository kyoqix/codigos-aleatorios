...
if type(environment.K27ECleanup) == "function" then
	pcall(environment.K27ECleanup)
end
...
local noRender = {
	...
	queuedInstances = setmetatable({}, { __mode = "k" }),
	queue = {},
	queueHead = 1,
	queueTail = 0,
	scanVersion = 0,
...
local function setRenderingQuality(enabled)
	pcall(function()
		local rendering = settings().Rendering

		if enabled then
			if noRender.renderQualityOriginal == nil then
				noRender.renderQualityOriginal = rendering.QualityLevel
			end
			rendering.QualityLevel = Enum.QualityLevel.Level01
		elseif noRender.renderQualityOriginal ~= nil then
			rendering.QualityLevel = noRender.renderQualityOriginal
			noRender.renderQualityOriginal = nil
		end
	end)
end

local function applyGlobalSettings()
	setRenderingQuality(noRender.enabled)
	...
	if terrain then
		if noRender.terrain then
			...
			lockProperty(terrain, "WaterTransparency", 1)
		else
			...
			unlockProperty(terrain, "WaterTransparency", true)
		end
	end
end

local function optimizeBasePart(instance, className)
	pcall(function()
...
local function processInstance(instance)
	...
	if noRender.processedInstances[instance] then
		return
	end

	noRender.processedInstances[instance] = true
	noRender.processed += 1

	local className = instance.ClassName

	if noRender.vfx and vfxDestroyClasses[className] then
		pcall(function()
	...
	if noRender.sfx then
		...
		if audioDestroyClasses[className] or instance:IsA("SoundEffect") then
			safeDestroy(instance)
			return
		end
	end
	...
	if noRender.parts then
		if instance:IsA("BasePart") then
			optimizeBasePart(instance, className)
			return
		end
...
local function clearQueue()
	table.clear(noRender.queue)
	noRender.queueHead = 1
	noRender.queueTail = 0
	noRender.queuedInstances = setmetatable({}, { __mode = "k" })
end

local function enqueueInstance(instance)
	if not noRender.enabled or not instance or noRender.queuedInstances[instance] then
		return
	end

	noRender.queuedInstances[instance] = true
	noRender.queueTail += 1
	noRender.queue[noRender.queueTail] = instance
end
...
local function processQueue()
	...
	local deadline = os.clock() + 0.0025
	local handled = 0

	while noRender.queueHead <= noRender.queueTail and handled < 500 do
		local index = noRender.queueHead
		local instance = noRender.queue[index]

		noRender.queue[index] = nil
		noRender.queueHead += 1
		handled += 1

		if instance then
			noRender.queuedInstances[instance] = nil
	...
	if noRender.queueHead > noRender.queueTail then
		clearQueue()
	end
end

local function disconnectNoRenderConnections()
	for index, connection in ipairs(noRender.connections) do
		connection:Disconnect()
		noRender.connections[index] = nil
	end
end

local function connectNoRender()
	disconnectNoRenderConnections()

	noRender.connections[1] = Workspace.DescendantAdded:Connect(enqueueInstance)
	noRender.connections[2] = Lighting.DescendantAdded:Connect(enqueueInstance)
	noRender.connections[3] = SoundService.DescendantAdded:Connect(enqueueInstance)
	noRender.connections[4] = RunService.Heartbeat:Connect(processQueue)
end

local function scanContainer(container, version)
	local descendants = container:GetDescendants()

	for index, instance in ipairs(descendants) do
		if not noRender.enabled or version ~= noRender.scanVersion then
			return false
		end
...
local function scanGame()
	...
	noRender.scanVersion += 1
	local version = noRender.scanVersion

	noRender.processed = 0
	noRender.destroyed = 0
	noRender.processedInstances = setmetatable({}, { __mode = "k" })

	if not scanContainer(Workspace, version) then
		return false
	end

	if not scanContainer(Lighting, version) then
		return false
	end

	if not scanContainer(SoundService, version) then
		return false
	end

	return noRender.enabled and version == noRender.scanVersion
end
...
local notifications = create("Frame", {
	...
	Position = UDim2.new(1, -282, 1, -252),
	BackgroundTransparency = 1
}, gui)
...
local function notify(text)
	...
	addStroke(notification, Color3.fromRGB(118, 118, 118), 1, 0.08)

	create("UIGradient", {
		Color = ColorSequence.new({
			ColorSequenceKeypoint.new(0, Color3.fromRGB(48, 48, 48)),
			ColorSequenceKeypoint.new(0.55, Color3.fromRGB(35, 35, 35)),
	...
	addCorner(icon, 7)
	addStroke(icon, Color3.fromRGB(145, 145, 145), 1, 0.18)
	...
	notification.Position = UDim2.fromOffset(24, 8)
	...
	if state then
		...
		task.delay(0.15, function()
			if not minimized and root.Parent then
				brand.Text = "K27E"
				brand.Size = UDim2.fromOffset(210, 20)
...
local function cleanup()
	...
	if environment.K27ECleanup == cleanup then
		environment.K27ECleanup = nil
	end
end

environment.K27ECleanup = cleanup
