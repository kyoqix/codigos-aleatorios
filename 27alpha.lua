...
create("UIGradient", {
	Color = ColorSequence.new({
		...
		ColorSequenceKeypoint.new(0.52, Color3.fromRGB(102, 75, 145)),
		ColorSequenceKeypoint.new(1, Color3.fromRGB(139, 105, 190))
	}),
	Rotation = 125
}, root)
...
local function showMusicNotification(trackName)
	...
	addStroke(notification, theme.accentBright, 1, 0.12)

	create("UIGradient", {
		Color = ColorSequence.new({
			ColorSequenceKeypoint.new(0, theme.selection),
			ColorSequenceKeypoint.new(0.55, theme.elevated),
	...
	addCorner(icon, 12)
	addStroke(icon, theme.accentBright, 1, 0.2)
	...
	create("TextLabel", {
		...
		Position = UDim2.fromOffset(74, 31),
		BackgroundTransparency = 1,
	...
	notification.Position = UDim2.new(0.5, 0, 0, 8)

	playTween(notification, {
		GroupTransparency = 0,
		Position = UDim2.new(0.5, 0, 0, 22)
	}, 0.95, Enum.EasingStyle.Quint)

	playTween(progress, {
		Size = UDim2.new(0, 0, 1, 0)
	}, 7.1, Enum.EasingStyle.Linear)
...
local function createMusicPlayerPanel(parent)
	createInfo(
		parent,
		"MUSIC WORKSPACE",
		"Place up to 15 .mp3, .ogg, .wav or .m4a files inside " .. musicPlayer.folder .. "."
	)

	local nowPlayingCard = createCard(parent, 78)
	...
	local seekCard = createCard(parent, 60)
	...
	addCorner(seekKnob, 6)
	addStroke(seekKnob, theme.accentBright, 2, 0.05)
	...
	local function setSeekFromX(positionX)
		...
		local ratio = math.clamp(
			(positionX - seekTrack.AbsolutePosition.X) / width,
			0,
			1
		)

		seekFill.Size = UDim2.new(ratio, 0, 1, 0)
		seekKnob.Position = UDim2.new(ratio, -6, 0.5, -6)
		seekMusic(ratio)
	end
	...
	local controlsCard = createCard(parent, 54)
	...
	local function createControl(text, order, callback)
		local button = create("TextButton", {
		...
		addCorner(button, 8)
		addStroke(button, theme.border, 1, 0.15)

		button.MouseButton1Click:Connect(callback)
		return button
	end

	local previousButton = createControl("PREV", 1, function()
	...
	local playButton = createControl("PLAY", 2, function()
	...
	local nextButton = createControl("NEXT", 3, function()
	...
	local stopButton = createControl("STOP", 4, function()
	...
	local volumeCard = createCard(parent, 64)
	...
	local volumeTrack = create("Frame", {
		Size = UDim2.new(1, -26, 0, 5),
		Position = UDim2.fromOffset(13, 46),
		BackgroundColor3 = theme.accentSoft,
		BorderSizePixel = 0
	}, volumeCard)

	addCorner(volumeTrack, 3)
	...
	local volumeKnob = create("Frame", {
		Size = UDim2.fromOffset(12, 12),
		Position = UDim2.new(musicPlayer.volume, -6, 0.5, -6),
		BackgroundColor3 = theme.text,
		BorderSizePixel = 0
	}, volumeTrack)

	addCorner(volumeKnob, 6)
	addStroke(volumeKnob, theme.accentBright, 2, 0.05)
	...
	local function setVolumeFromX(positionX)
		...
		local ratio = math.clamp(
			(positionX - volumeTrack.AbsolutePosition.X) / width,
			0,
			1
		)
		...
		volumeKnob.Position = UDim2.new(ratio, -6, 0.5, -6)
	...
	local libraryCard = createCard(parent, 240)
	...
	addCorner(libraryList, 8)
	addStroke(libraryList, theme.border, 1, 0.08)
	...
	local function updateState()
		...
		if sound then
			playbackStatus.Text = musicPlayer.paused and "Paused" or "Playing"
		else
	...
	local function renderLibrary()
		...
		for index, track in ipairs(musicPlayer.tracks) do
			...
			addCorner(row, 7)
			addStroke(row, theme.border, 1, 0.18)
