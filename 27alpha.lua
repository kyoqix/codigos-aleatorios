...
local theme = {}
for key, value in pairs(themePresets.Violet) do
	theme[key] = value
end

local currentThemeName = "Violet"
local animationDurationScale = 1
local interfaceScaleMultiplier = 1
local borderIntensity = 1
local themeBindings = setmetatable({}, {__mode = "k"})
local gradientBindings = setmetatable({}, {__mode = "k"})
local strokeBaseTransparency = setmetatable({}, {__mode = "k"})

local function getThemeKey(color)
...
local function bindGradient(object, sequence)
	local entries = {}

	for _, keypoint in ipairs(sequence.Keypoints) do
		table.insert(entries, {
			time = keypoint.Time,
			key = getThemeKey(keypoint.Value),
			color = keypoint.Value
		})
	end

	gradientBindings[object] = entries
end

local function renderStrokeTransparency(value)
	return 1 - ((1 - math.clamp(value, 0, 1)) * borderIntensity)
end
...
local function create(className, properties, parent)
	...
	for property, value in pairs(properties or {}) do
		...
		bindThemeProperty(object, property, value)
	end

	if className == "UIGradient" and properties and properties.Color then
		bindGradient(object, properties.Color)
	end
...
local function playTween(object, properties, duration, style, direction)
	...
	for property, value in pairs(properties) do
		...
		if object:IsA("UIStroke") and property == "Transparency" then
			strokeBaseTransparency[object] = value
			tweenProperties[property] = renderStrokeTransparency(value)
		else
	...
	local animation = TweenService:Create(
		object,
		TweenInfo.new(
			math.max(0.01, (duration or 0.2) * animationDurationScale),
...
local function addStroke(object, color, thickness, transparency)
	...
	local stroke = create("UIStroke", {
		...
		Transparency = renderStrokeTransparency(baseTransparency)
	}, object)

	strokeBaseTransparency[stroke] = baseTransparency
	return stroke
end
...
local function applyTheme(name)
	...
	for gradient, entries in pairs(gradientBindings) do
...
local function setBorderIntensity(value)
	borderIntensity = math.clamp(value, 0, 1)

	for stroke, baseTransparency in pairs(strokeBaseTransparency) do
		if stroke and stroke.Parent then
			stroke.Transparency = renderStrokeTransparency(baseTransparency)
		end
	end
end
...
local INTERFACE_CONFIG_FILE = "K27E_INTERFACE_CONFIG.json"
local interfaceConfigSaveVersion = 0

local function saveInterfaceConfig()
	local data = {
		theme = currentThemeName,
		scale = interfaceScaleMultiplier,
		animationSpeed = 1 / animationDurationScale,
	...
	return pcall(writeFile, INTERFACE_CONFIG_FILE, encoded)
end

local function queueInterfaceConfigSave()
	interfaceConfigSaveVersion += 1
	local version = interfaceConfigSaveVersion

	task.delay(0.35, function()
		if version == interfaceConfigSaveVersion then
			saveInterfaceConfig()
		end
	end)
end

local function loadInterfaceConfig()
	local loaded
	...
	if readFile then
		...
		if isFile then
			local checked, exists = pcall(isFile, INTERFACE_CONFIG_FILE)
			canRead = checked and exists == true
		end

		if canRead then
			local readSuccessfully, encoded = pcall(readFile, INTERFACE_CONFIG_FILE)
	...
	if type(loaded.theme) == "string" and themePresets[loaded.theme] then
		applyTheme(loaded.theme)
	end

	interfaceScaleMultiplier = math.clamp(tonumber(loaded.scale) or 1, 0.75, 1.15)
	local animationSpeed = math.clamp(tonumber(loaded.animationSpeed) or 1, 0.5, 2)
	animationDurationScale = 1 / animationSpeed
	borderIntensity = math.clamp(tonumber(loaded.borderIntensity) or 1, 0.15, 1)
end
...
local function updateScale()
	...
	scale.Scale = math.clamp(target * interfaceScaleMultiplier, 0.56, 1.2)
end
...
local function createThemePicker(parent, callback)
	local order = {"Violet", "Indigo", "Rose", "Cyan"}
	local buttons = {}
	local selected = currentThemeName
...
local themePicker = createThemePicker(settingsPage, function(name)
	if applyTheme(name) then
		queueInterfaceConfigSave()
		notify(name .. " theme applied")
	end
end)

local interfaceScaleSlider = createSlider(settingsPage, "Interface size", 0.75, 1.15, interfaceScaleMultiplier, 2, function(value)
	interfaceScaleMultiplier = value
	updateScale()
	queueInterfaceConfigSave()
end)

local animationSpeedSlider = createSlider(settingsPage, "Animation speed", 0.5, 2, 1 / animationDurationScale, 2, function(value)
	animationDurationScale = 1 / value
	queueInterfaceConfigSave()
end)

local borderIntensitySlider = createSlider(settingsPage, "Border intensity", 0.15, 1, borderIntensity, 2, function(value)
	setBorderIntensity(value)
	queueInterfaceConfigSave()
end)

themePicker.set(currentThemeName, false)
setBorderIntensity(borderIntensity)
...
createButton(settingsPage, "Reset interface settings", function()
	interfaceScaleMultiplier = 1
	animationDurationScale = 1
	setBorderIntensity(1)
	applyTheme("Violet")
	interfaceScaleSlider.set(1, false)
	animationSpeedSlider.set(1, false)
	borderIntensitySlider.set(1, false)
	themePicker.set("Violet", false)
	updateScale()
	saveInterfaceConfig()
