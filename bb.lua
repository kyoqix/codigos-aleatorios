...
function Runtime:Unload()
    ...
    local hookStateV3 = ENV.__EagleHubXRemoteHookStateV3
    if hookStateV3 and hookStateV3.runtime == self then
        hookStateV3.runtime = nil
    end
...
local function hasParryArgumentShape(args)
    ...
    if typeof(args[3]) == "Vector3" and typeof(args[4]) == "CFrame" then
        return true
    end
    ...
    return hasVector and hasCFrame and hasTable
end

function Runtime:ObserveRemote(remote, method, args, rawMethod)
    if not self.active then
        return
    end
    ...
    if self.remoteHooked and self.parryRemote ~= remote and not named then
        return
    end
    ...
    self.parryMethod = method
    self.parryRawMethod = rawMethod
    self.parryArgs = copy
...
local previousHookState = ENV.__EagleHubXRemoteHookState
if previousHookState then
    previousHookState.runtime = nil
end

local previousHookStateV3 = ENV.__EagleHubXRemoteHookStateV3
if previousHookStateV3 then
    previousHookStateV3.runtime = nil
end
...
ENV.__EagleHubXRemoteHookStateV3 = HookState
...
local function installIndexHook()
    ...
    local success = pcall(function()
        ...
        metatable.__index = newcclosure(function(self, key)
            local isFireServer = key == "FireServer" and typeof(self) == "Instance" and self:IsA("RemoteEvent")
            local isInvokeServer = key == "InvokeServer" and typeof(self) == "Instance" and self:IsA("RemoteFunction")

            if isFireServer or isInvokeServer then
                local rawMethod = rawIndex(self, key)
                ...
                return function(instance, ...)
                    ...
                    if runtime and runtime.active then
                        ...
                        pcall(function()
                            runtime:ObserveRemote(instance, key, arguments, rawMethod)
                        end)
                    end

                    return rawMethod(instance, ...)
                end
            end
    ...
    return success and HookState.mode == "index"
end
...
local function installNamecallHook()
    ...
    local success = pcall(function()
        oldNamecall = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
            ...
            if runtime and runtime.active and (method == "FireServer" or method == "InvokeServer") then
                local arguments = table.pack(...)
                pcall(function()
                    runtime:ObserveRemote(self, method, arguments, nil)
                end)
            end
...
local function resolveNamedParryRemote()
    ...
    if bestScore > 0 then
        ...
        Runtime.parryMethod = bestRemote:IsA("RemoteFunction") and "InvokeServer" or "FireServer"
    end
end
...
local function fireParry(playAnimation)
    ...
    local success = pcall(function()
        local rawMethod = Runtime.parryRawMethod
        local method = Runtime.parryMethod

        if type(rawMethod) == "function" then
            rawMethod(remote, table.unpack(args, 1, args.n))
        elseif method == "InvokeServer" and remote:IsA("RemoteFunction") then
...
function SkinChanger:SetParryHookEnabled(enabled)
    local parrySuccessAll = findPath(ReplicatedStorage, {"Remotes", "ParrySuccessAll"})

    if enabled then
        if not parrySuccessAll or not parrySuccessAll:IsA("RemoteEvent") then
            return
        end

        if not self.originalParryConnection or not self.originalParryFunction then
            return
        end

        if not self.replacementConnection then
        ...
        if self.replacementConnection then
            pcall(function()
                self.replacementConnection:Disconnect()
            end)
            self.replacementConnection = nil
        end
...
function SkinChanger:SetEnabled(enabled)
    State.skinChanger = enabled
    self:SetParryHookEnabled(enabled)
...
function SkinChanger:Update()
    ...
    self:SetParryHookEnabled(State.skinChanger)
    self:Equip()
end
...
function SkinChanger:Initialize()
    ...
    task.spawn(function()
        ...
        if parrySuccessAll and parrySuccessAll:IsA("RemoteEvent") and getconnections and getinfo then
            ...
            if self.originalParryFunction and State.skinChanger then
                self:SetParryHookEnabled(true)
            end
        end
...
function SkinChanger:Unload()
    self.active = false
    self:SetParryHookEnabled(false)
end
...
local function setSpamActive(enabled)
    if enabled and (not Runtime.remoteHooked or not Runtime.parryArgs) then
...
Runtime:Connect(RunService.Heartbeat, function(deltaTime)
    ...
    if root and ball and ball.Parent then
        ...
        if State.autoParry and Runtime.remoteHooked and Runtime.parryArgs and not shouldBlockAbilityParry() then
    ...
    if State.spamActive and Runtime.remoteHooked and Runtime.parryArgs then
