...
interface.release = {
	...
	channel = "Stable",
	changelog = {
		"Moved the activity console to Debug",
		"Simplified Home to show only the update log",
		"Refined sidebar labels and console visuals"
	}
}
...
function interface.autoConfig.get(section, key, defaultValue)
	if not interface.autoConfig.enabled then
		return defaultValue
	end

	local sectionData = type(interface.autoConfig.data) == "table" and interface.autoConfig.data[section] or nil
	local value = type(sectionData) == "table" and sectionData[key] or nil
...
function interface.autoConfig.load()
	local loaded
	...
	if readFile then
		...
		if isFile then
			local checked, exists = pcall(isFile, interface.autoConfig.file)
			canRead = checked and exists == true
		end

		if canRead then
			local readSuccessfully, encoded = pcall(readFile, interface.autoConfig.file)
			...
			if readSuccessfully and type(encoded) == "string" then
				...
				if decodedSuccessfully and type(decoded) == "table" then
					loaded = decoded
					interface.autoConfig.lastEncoded = encoded
				end
			end
		end
	end

	if type(loaded) ~= "table" and type(executorEnvironment) == "table" then
		local sessionConfig = rawget(executorEnvironment, "K27EAutoConfig")
	...
	if type(loaded) == "table" then
		interface.autoConfig.data = loaded
		interface.autoConfig.enabled = loaded.enabled == true
	end
	...
	local savedInterface = interface.autoConfig.data.interface

	if type(savedInterface) == "table" then
		if type(savedInterface.theme) == "string" and interface.presets[savedInterface.theme] then
			interface.applyTheme(savedInterface.theme)
		end

		interface.scale = math.clamp(tonumber(savedInterface.scale) or interface.scale, 0.75, 1.15)
		interface.animationScale = 1 / math.clamp(
			tonumber(savedInterface.animationSpeed) or (1 / interface.animationScale),
			0.5,
			2
		)
		interface.borderIntensity = math.clamp(
			tonumber(savedInterface.borderIntensity) or interface.borderIntensity,
			0.15,
			1
		)
	end
...
if interface.autoConfig.enabled then
	noRender.vfx = interface.autoConfig.get("noRender", "vfx", noRender.vfx) == true
	noRender.sfx = interface.autoConfig.get("noRender", "sfx", noRender.sfx) == true
	noRender.lighting = interface.autoConfig.get("noRender", "lighting", noRender.lighting) == true
	noRender.parts = interface.autoConfig.get("noRender", "parts", noRender.parts) == true
	noRender.terrain = interface.autoConfig.get("noRender", "terrain", noRender.terrain) == true
	screenStretch.factor = math.clamp(
		tonumber(interface.autoConfig.get("screenStretch", "factor", screenStretch.factor)) or screenStretch.factor,
		screenStretch.minimum,
		screenStretch.maximum
	)
	musicPlayer.volume = math.clamp(
		tonumber(interface.autoConfig.get("music", "volume", musicPlayer.volume)) or musicPlayer.volume,
		0,
		1
	)
	musicPlayer.looped = interface.autoConfig.get("music", "looped", musicPlayer.looped) == true
end
...
interface.autoConfig.rendering = {
...
function interface.autoConfig.rendering.setEnabled(state)
	state = state == true

	if interface.autoConfig.rendering.Connections.RuntimeChildAdded then
		interface.autoConfig.rendering.Connections.RuntimeChildAdded:Disconnect()
		interface.autoConfig.rendering.Connections.RuntimeChildAdded = nil
	end

	interface.autoConfig.rendering.HideServerRendering = false

	if not state then
		return true
	end

	local runtime = Workspace:FindFirstChild("Runtime") or Workspace:WaitForChild("Runtime", 5)

	if not runtime then
		return false, "Blade Ball Runtime folder was not found"
	end

	interface.autoConfig.rendering.HideServerRendering = true
	interface.autoConfig.rendering.Connections.RuntimeChildAdded = runtime.ChildAdded:Connect(function(child)
		if interface.autoConfig.rendering.HideServerRendering then
			interface.autoConfig.rendering.Debris:AddItem(child, 0)
		end
	end)
...
if type(executorEnvironment) == "table" then
	interface.autoConfig.previousRendering = rawget(executorEnvironment, "AchaoticRendering")

	if type(interface.autoConfig.previousRendering) == "table"
		and type(interface.autoConfig.previousRendering.Connections) == "table" then
		for _, connection in pairs(interface.autoConfig.previousRendering.Connections) do
			pcall(function()
				connection:Disconnect()
			end)
		end
	...
	rawset(executorEnvironment, "AchaoticRendering", interface.autoConfig.rendering)
end
...
local subtitle = create("TextLabel", {
	...
	BackgroundTransparency = 1,
	Text = "v" .. interface.release.version .. "  /  performance control panel",
...
do
	local profileCard = create("Frame", {
		...
		Position = UDim2.new(0, 9, 1, -49),
		BackgroundColor3 = theme.surfaceAlt,
		BorderSizePixel = 0
	}, sidebar)

	addCorner(profileCard, 9)
	addStroke(profileCard, theme.border, 1, 0.18)
	...
	addStroke(avatarFrame, theme.borderActive, 1, 0.2)
	...
	local maskedName = username:sub(1, 2) .. string.rep("*", math.max(#username - 2, 0))
...
function interface.console.getLevelColor(level)
	level = string.upper(tostring(level or "INFO"))

	local colors = {
		ERROR = Color3.fromRGB(255, 128, 151),
		WARN = Color3.fromRGB(255, 202, 122),
		UPDATE = theme.accentBright,
		SYSTEM = Color3.fromRGB(145, 211, 255),
		CONFIG = Color3.fromRGB(203, 176, 255),
		INFO = theme.accent
	}

	return colors[level] or theme.accent
end

function interface.console.render()
	local console = interface.console

	if not console.list or not console.list.Parent then
		return
	end

	for _, child in ipairs(console.list:GetChildren()) do
		if child.Name == "LogEntry" or child.Name == "EmptyState" then
			child:Destroy()
		end
	end

	if #console.entries == 0 then
		create("TextLabel", {
			Name = "EmptyState",
		...
		for index, entry in ipairs(console.entries) do
			local levelColor = interface.console.getLevelColor(entry.level)
			local row = create("Frame", {
				Name = "LogEntry",
				Size = UDim2.new(1, -6, 0, 31),
				BackgroundColor3 = index % 2 == 0 and theme.surfaceAlt or theme.input,
			...
			create("TextLabel", {
				Size = UDim2.fromOffset(48, 31),
				Position = UDim2.fromOffset(67, 0),
				BackgroundTransparency = 1,
				Text = entry.level,
	...
	if console.counter and console.counter.Parent then
		console.counter.Text = tostring(#console.entries)
	end

	task.defer(function()
		if console.list and console.list.Parent then
			console.list.CanvasPosition = Vector2.new(
				0,
				math.max(0, console.list.AbsoluteCanvasSize.Y - console.list.AbsoluteWindowSize.Y)
			)
		end
	end)
end

function interface.console.add(message, level)
	local console = interface.console
	...
	while #console.entries > console.maxEntries do
		table.remove(console.entries, 1)
	end

	interface.console.render()
end

function interface.console.clear()
	interface.console.entries = {}
	interface.console.render()
end

function interface.console.mount(parent)
	local console = interface.console
	local card = create("Frame", {
		Size = UDim2.new(1, -2, 0, 206),
		BackgroundColor3 = theme.surface,
		BorderSizePixel = 0,
		ClipsDescendants = true
	}, parent)
	...
	addCorner(card, 9)
	addStroke(card, theme.border, 1, 0.28)
	...
	create("TextLabel", {
		Size = UDim2.new(1, -154, 0, 14),
		Position = UDim2.fromOffset(54, 24),
		BackgroundTransparency = 1,
		Text = "Live hub activity",
		TextColor3 = theme.subtext,
		TextSize = 9,
		Font = Enum.Font.SourceSans,
		TextXAlignment = Enum.TextXAlignment.Left
	...
	addCorner(counterPill, 6)
	addStroke(counterPill, theme.border, 1, 0.38)

	console.counter = create("TextLabel", {
		Size = UDim2.fromScale(1, 1),
		BackgroundTransparency = 1,
		Text = "0",
		TextColor3 = theme.subtext,
	...
	addCorner(clearButton, 6)
	addStroke(clearButton, theme.border, 1, 0.38)

	clearButton.MouseEnter:Connect(function()
		playTween(clearButton, {
			BackgroundColor3 = theme.hover,
			TextColor3 = theme.text
		}, 0.14)
	end)
	...
	console.list = create("ScrollingFrame", {
	...
	addCorner(console.list, 7)
	addStroke(console.list, theme.border, 1, 0.42)
...
local function notify(text, level)
	interface.console.add(text, level or "INFO")
...
function interface.autoConfig.collect()
	return {
		enabled = interface.autoConfig.enabled,
		noRender = {
			enabled = noRender.enabled,
			vfx = noRender.vfx,
			sfx = noRender.sfx,
			lighting = noRender.lighting,
			parts = noRender.parts,
			terrain = noRender.terrain
		},
		screenStretch = {
			enabled = screenStretch.enabled,
			factor = screenStretch.factor
		},
		bladeBall = {
			hideServerRendering = interface.autoConfig.rendering.HideServerRendering
		},
		appearance = {
			headless = appearance.headless,
			korblox = appearance.korblox
		},
		debug = {
			fps = debugController.getFps(),
			ping = debugController.getPing()
		},
		music = {
			volume = musicPlayer.volume,
			looped = musicPlayer.looped
		},
		interface = {
			theme = interface.themeName,
			scale = interface.scale,
...
function interface.autoConfig.save(force)
	local data = interface.autoConfig.collect()
	local encodedSuccessfully, encoded = pcall(function()
		return HttpService:JSONEncode(data)
	end)
	...
	interface.autoConfig.data = data

	if type(executorEnvironment) == "table" then
		rawset(executorEnvironment, "K27EAutoConfig", data)
	end

	if not force and encoded == interface.autoConfig.lastEncoded then
		return true
	end
	...
	return pcall(writeFile, interface.autoConfig.file, encoded)
end

function interface.autoConfig.setEnabled(state)
	interface.autoConfig.enabled = state == true
	interface.autoConfig.save(true)
end

function interface.autoConfig.start()
	if interface.autoConfig.running then
		return
	end

	interface.autoConfig.running = true
	interface.autoConfig.stopped = false

	task.spawn(function()
		while interface.autoConfig.running and gui.Parent do
			task.wait(0.65)

			if interface.autoConfig.enabled and not interface.autoConfig.applying then
				interface.autoConfig.save(false)
			end
		end
	end)
end

function interface.autoConfig.stop(saveCurrent)
	if interface.autoConfig.stopped then
		return
	end

	interface.autoConfig.stopped = true
	interface.autoConfig.running = false

	if saveCurrent and interface.autoConfig.enabled and not interface.autoConfig.applying then
		interface.autoConfig.save(true)
	end
end
...
createTab("Controls", "Boost", 2)
createTab("FFlags", "Flags", 3)
createTab("Misc", "Misc", 4)
createTab("Debug", "Debug", 5)
createTab("Settings", "Config", 6)

createSection(homePage, "Update log")

local updateCard = create("Frame", {
	Size = UDim2.new(1, -2, 0, 146),
	BackgroundColor3 = theme.surface,
	BorderSizePixel = 0
}, homePage)

addCorner(updateCard, 9)
addStroke(updateCard, theme.border, 1, 0.28)

local updateAccent = create("Frame", {
...
addCorner(updateAccent, 2)

create("TextLabel", {
	Size = UDim2.new(1, -32, 0, 20),
	Position = UDim2.fromOffset(24, 8),
	BackgroundTransparency = 1,
	Text = "K27E  v" .. interface.release.version,
	TextColor3 = theme.text,
	TextSize = 13,
	Font = Enum.Font.SourceSansSemibold,
	TextXAlignment = Enum.TextXAlignment.Left
...
create("TextLabel", {
	...
	BackgroundTransparency = 1,
	Text = interface.release.channel .. "  /  build " .. tostring(interface.release.build),
	TextColor3 = theme.subtext,
	TextSize = 9,
	Font = Enum.Font.SourceSans,
	TextXAlignment = Enum.TextXAlignment.Left
...
for index, line in ipairs(interface.release.changelog) do
	local row = create("Frame", {
		Size = UDim2.new(1, -24, 0, 26),
		Position = UDim2.fromOffset(12, 50 + ((index - 1) * 28)),
		BackgroundColor3 = theme.input,
		BackgroundTransparency = 0.2,
		BorderSizePixel = 0
	}, updateCard)
...
local mainToggle
mainToggle = createToggle(
	controlsPage,
	"No Render",
	interface.autoConfig.get("noRender", "enabled", false),
	function(value)
...
createSection(controlsPage, "Blade Ball")

interface.controls.hideServerRendering = createToggle(
	controlsPage,
	"Hide server rendering",
	interface.autoConfig.get("bladeBall", "hideServerRendering", false),
	function(value)
		local success, errorMessage = interface.autoConfig.rendering.setEnabled(value)

		if not success then
			interface.controls.hideServerRendering.set(false, false)
			notify(errorMessage)
			return
		end

		notify(value and "Server rendering hidden" or "Server rendering restored")
	end
)
...
createInfo(
	controlsPage,
	"SERVER RUNTIME",
	"Deletes every new child added to workspace.Runtime while enabled."
)

createSection(controlsPage, "Cleanup")

createToggle(controlsPage, "Visual effects", noRender.vfx, function(value)
	noRender.vfx = value
	refreshNoRender()
end)

createToggle(controlsPage, "Sounds and effects", noRender.sfx, function(value)
	noRender.sfx = value
	refreshNoRender()
end)

createToggle(controlsPage, "Lighting and post effects", noRender.lighting, function(value)
	noRender.lighting = value
	refreshNoRender()
end)

createToggle(controlsPage, "Part shadows and reflectance", noRender.parts, function(value)
	noRender.parts = value
	refreshNoRender()
end)

createToggle(controlsPage, "Terrain decoration and water", noRender.terrain, function(value)
...
stretchToggle = createToggle(
	controlsPage,
	"Screen stretch",
	interface.autoConfig.get("screenStretch", "enabled", false),
	function(value)
...
local headlessToggle = createToggle(
	miscPage,
	"Headless",
	interface.autoConfig.get("appearance", "headless", false),
	function(value)
...
local korbloxToggle = createToggle(
	miscPage,
	"Korblox",
	interface.autoConfig.get("appearance", "korblox", false),
	function(value)
...
createSection(debugPage, "Console")
interface.console.mount(debugPage)

createSection(debugPage, "Overlay")

interface.controls.debugFps = createToggle(
	debugPage,
	"Show FPS",
	interface.autoConfig.get("debug", "fps", false),
	function(value)
...
interface.controls.debugPing = createToggle(
	debugPage,
	"Show ping",
	interface.autoConfig.get("debug", "ping", false),
	function(value)
...
createSection(settingsPage, "Autoconfig")

interface.controls.autoConfig = createToggle(
	settingsPage,
	"Apply saved config on launch",
	interface.autoConfig.enabled,
	function(value)
		interface.autoConfig.setEnabled(value)
		notify(value and "Autoconfig enabled" or "Autoconfig disabled")
	end
)
...
createInfo(
	settingsPage,
	"AUTOCONFIG",
	"Automatically stores active hub options in K27E_AUTOCONFIG.json. FFlag editor content is excluded."
)

createSection(settingsPage, "Release")

createInfo(
	settingsPage,
	"K27E v" .. interface.release.version .. "  /  " .. interface.release.channel,
	"Build " .. tostring(interface.release.build) .. ". Version tracking is ready for future releases."
)

createInfo(
	settingsPage,
	"LATEST CHANGES",
	table.concat(interface.release.changelog, "  •  ")
)

createButton(settingsPage, "Send release notes to console", function()
	for _, line in ipairs(interface.release.changelog) do
		interface.console.add(line, "UPDATE")
	end

	switchPage("Debug")
	notify("Release notes added to Debug console", "UPDATE")
end)
...
createButton(settingsPage, "Disable and close", function()
	interface.autoConfig.stop(true)
	interface.autoConfig.rendering.setEnabled(false)
	stopMusic()
...
if interface.autoConfig.enabled then
	interface.autoConfig.applying = true
	setScreenStretchEnabled(interface.autoConfig.get("screenStretch", "enabled", false))
	setHeadlessEnabled(interface.autoConfig.get("appearance", "headless", false))
	setKorbloxEnabled(interface.autoConfig.get("appearance", "korblox", false))
	debugController.setFps(interface.autoConfig.get("debug", "fps", false))
	debugController.setPing(interface.autoConfig.get("debug", "ping", false))

	interface.autoConfig.renderingSuccess = interface.autoConfig.rendering.setEnabled(
		interface.autoConfig.get("bladeBall", "hideServerRendering", false)
	)

	if not interface.autoConfig.renderingSuccess and interface.controls.hideServerRendering then
		interface.controls.hideServerRendering.set(false, false)
	end

	setNoRenderEnabled(interface.autoConfig.get("noRender", "enabled", false))
	interface.autoConfig.applying = false
end

interface.autoConfig.start()
updateStatus()

interface.console.add("K27E v" .. interface.release.version .. " initialized", "SYSTEM")
interface.console.add(
	interface.autoConfig.enabled and "Saved configuration applied" or "Autoconfig is disabled",
	"CONFIG"
)
...
task.delay(0.4, function()
	notify(interface.autoConfig.enabled and "K27E loaded with autoconfig" or "K27E loaded")
end)
